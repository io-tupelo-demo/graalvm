...
This is a full OpenJDK-8 distribution.  I always install packages like this in `/opt`, so the final
...
# create a symbolic link so our ~/.bashrc file doesn't need to change when we upgrade graalvm versions
/opt > sudo ln -s graalvm-ce-19.2.1 graalvm
/opt > d *
lrwxr-xr-x  1 root     wheel               17 Nov  8 11:47 graalvm@ -> graalvm-ce-19.2.1
drwxr-xr-x  3 r634165  RBSWA\Domain Users  96 Nov  8 11:45 graalvm-ce-19.2.1/
```
